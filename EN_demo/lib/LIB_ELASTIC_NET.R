...
# #######################################################################################################################################
# ElasticNet prediciton
# #######################################################################################################################################
# IN:  model  <= trained model which is the output from 'trainEN(...)'
# OUT: prediction <= Prediction of feature with given model
predictEN <- function(model, feat) {
  return(predict(model[["model"]],
                 type="response",newx=as.matrix(feat))[,model[["model"]]$lambda == model[["lambda"]]])
}